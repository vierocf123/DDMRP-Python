...
def def_calcular_projecao(data_ref, lista_DDMRP_completo, lista_DDMRP_completo_DIAS,
    ...
    for index, row in df_parametros.iterrows():
        ...
        if tipo_de_planejamento == "PMP":
            ...
            if dias_antecipacao > 0:
                ...
                for data_ref_fluxo in lista_DDMRP_fluxo:
                    ...
                    antecipacao_i, antecipacao_f = calcular_range(v_dias_uteis=lista_DDMRP_fluxo_DIAS,
                                                                  ...
                                                                  sentido=False,
                                                                  excludente=True)
                    if antecipacao_i <= -1: antecipacao_i = 0
            ...
            df_parametros.loc[index,"QTD_ATRASO_ADJ"] = demanda_qualificada_atraso
            ...
            df_parametros.loc[index, "DEMANDA_HOJE_REAL"] = qtd_atraso +  np.sum(arr_demanda[index][0])
            df_parametros.loc[index, "DEMANDA_HOJE"] = np.sum(arr_DEMANDA_QUALIFICADA_aux[:1])
            df_parametros.loc[index,"DEMANDA_PF"] = np.sum(arr_DEMANDA_QUALIFICADA_aux[1:periodo_firme_f + 2])
            df_parametros.loc[index,"NIP_SUGERIDA"] = arr_planejado_entrega[index][periodo_firme_f + 1]
            #Se existe demanda após o PF
            if arr_planejado_entrega[index][periodo_firme_f+1] > 0:
                df_parametros.loc[index,"DATA_ENTREGA"] = lista_DDMRP_fluxo[producao_fim_pf_d1]
            
            #Fluxo liquido
            df_parametros.loc[index,"FLUXO_LIQUIDO"] = arr_FLUXO_LIQUIDO_aux[periodo_firme_f + 1] - arr_planejado_entrega[index][periodo_firme_f + 1]
            
            #Calcular Prioridade
            if (df_parametros.loc[index,"QTD_ATRASO_ADJ"] + df_parametros.loc[index, "DEMANDA_HOJE"] + df_parametros.loc[index,"DEMANDA_PF"] == 0):
                df_parametros.loc[index, "PRIORIDADE%"] = 1
            else:
                df_parametros.loc[index, "PRIORIDADE%"] = (df_parametros.loc[index, "FLUXO_LIQUIDO"] + (df_parametros.loc[index,"QTD_ATRASO_ADJ"] + df_parametros.loc[index, "DEMANDA_HOJE"] + df_parametros.loc[index,"DEMANDA_PF"])) / (df_parametros.loc[index,"QTD_ATRASO_ADJ"] + df_parametros.loc[index, "DEMANDA_HOJE"] + df_parametros.loc[index,"DEMANDA_PF"])
