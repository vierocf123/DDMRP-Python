...
def def_exportar_projecao_SAP(arr_PLANEJADO_PROJECAO, datas_projecao, lista_materiais, df_dado_mestre, base_dir, folder, data_ref, df_parametros):
    ...
    #Remover itens PMP com subida para hoje
    df_parametros = df_parametros[["Material", "NIP_SUGERIDA", "DATA_ENTREGA", "TIPO_PLANEJAMENTO"]].copy()
    df_parametros = df_parametros[(df_parametros["TIPO_PLANEJAMENTO"].eq("PMP") &
                                   df_parametros["DATA_ENTREGA"].notna() &
                                   df_parametros["DATA_ENTREGA"].astype(str).str.strip().ne(""))]
    df_parametros = df_parametros[["Material", "DATA_ENTREGA"]]
    df_parametros = df_parametros.rename(columns={"Material": "MATERIAL", "DATA_ENTREGA": "DATA"})

    df_projecao_planejado = (df_projecao_planejado
                             .merge(
                                 df_parametros[["MATERIAL", "DATA"]].drop_duplicates(),
                                 on=["MATERIAL", "DATA"],
                                 how="left",
                                 indicator=True
                                 )
...
def exportar_projecao_PBI(lista_DDMRP_fluxo, lista_materiais, data_ref, df_parametros, df_custo, base_dir, folder,
    ...
    #Agrupar informações de D0 ##################################################
    
    ##TOG
    df_TOG = pd.DataFrame(arr_TOG, columns=lista_DDMRP_fluxo, index=lista_materiais)
    ...
    ##TOY
    df_TOY = pd.DataFrame(arr_TOY, columns=lista_DDMRP_fluxo, index=lista_materiais)
    ...
    ##TOR
    df_TOR = pd.DataFrame(arr_TOR, columns=lista_DDMRP_fluxo, index=lista_materiais)
    ...
    ##ADU
    df_ADU = pd.DataFrame(arr_ADU, columns=lista_DDMRP_fluxo, index=lista_materiais)
    ...
    ##ESTOQUE
    df_EST = pd.DataFrame(arr_ESTOQUE, columns=lista_DDMRP_fluxo, index=lista_materiais)
    ...
    ##FLUXO LIQUIDO
    df_FLU = pd.DataFrame(arr_FLUXO_LIQUIDO, columns=lista_DDMRP_fluxo, index=lista_materiais)
    ...
    dfs_projecao = [df_TOG, df_TOY, df_TOR, df_DAF, df_ADU, df_EST, df_PLA, df_PLA_ENT, df_FLU, df_NIPs]

    df_PROJECAO = reduce(
        lambda left, right: pd.merge(left, right, on=['Material', 'DATA'], how='inner'),
        dfs_projecao
    )   
    ...
    s_mapa = df_parametros.set_index('Material')['TIPO_PLANEJAMENTO']
    df_PROJECAO['TIPO_PLANEJAMENTO'] = df_PROJECAO['Material'].map(s_mapa).fillna(0)

    #Atribuir classificação de Status
    mask_DDMRP = df_PROJECAO['TIPO_PLANEJAMENTO'].eq('DDMRP')
    mask_PMP = df_PROJECAO['TIPO_PLANEJAMENTO'].eq('PMP')

    #Condicional DDMRP
    tog_safe = df_PROJECAO.loc[mask_DDMRP, 'TOG'].replace(0, np.nan)
    ratio     = df_PROJECAO.loc[mask_DDMRP, 'FLUXO_LIQUIDO']/tog_safe
    tor_ratio = df_PROJECAO.loc[mask_DDMRP, 'TOR']/tog_safe
    toy_ratio = df_PROJECAO.loc[mask_DDMRP, 'TOY']/tog_safe

    conds_DDMRP = [df_PROJECAO.loc[mask_DDMRP, 'ADU'].fillna(0).eq(0),
                   ratio.le(0),
                   ratio.le(tor_ratio),
                   ratio.le(toy_ratio),
                   ratio.le(1),]
    
    choices_DDMRP = ['SEM BUFFER', 'RUPTURA', 'RISCO', 'BAIXO', 'NORMAL']

    df_PROJECAO.loc[mask_DDMRP, 'STATUS'] = np.select(conds_DDMRP,
                                                      choices_DDMRP,
                                                      default='EXCESSO')

    #Condicional PMP
    fluxo = df_PROJECAO.loc[mask_PMP, 'FLUXO_LIQUIDO']

    conds_PMP = [fluxo.lt(0),
                 fluxo.eq(0),
                 fluxo.gt(0)]
    
    choices_PMP = ['RUPTURA', 'NORMAL', 'EXCESSO']

    df_PROJECAO.loc[mask_PMP, 'STATUS'] = np.select(conds_PMP,
                                                    choices_PMP,
                                                    default=np.nan)
    ...
    df_PROJECAO = df_PROJECAO[['DATA_REF','MATERIAL','DATA','PLANEJADO','FLUXO_LIQUIDO','ADU',
                            ...
                            'ADU_LIMPA','DAF','NIPs','PLANEJADO_ENTREGA','TIPO_MATERIAL', 'TIPO_PLANEJAMENTO']]

    #Exportar separadamente por Tipo de Material e Estretegia

    #Exportar pro estrategia
    df_filtrado = df_PROJECAO[df_PROJECAO['TIPO_PLANEJAMENTO'] == "PMP"]
    nome_arquivo = f"HISTORICO_FLUXO_SEMANAL_PMP.xlsx"
    caminho_completo = os.path.join(base_dir, folder, nome_arquivo)
    df_filtrado.to_excel(caminho_completo, sheet_name="Planilha1", index=False)

    #Exportar por cateogoria
    categorias = ['MATERIA-PRIMA', 'CONSUMIVEL', 'TERCERIZADO', 'MANUFATURADO', 'PRODUTO-PRONTO']
    for cat in categorias:
        df_filtrado = df_PROJECAO[(df_PROJECAO['TIPO_MATERIAL'] == cat) & (df_PROJECAO['TIPO_PLANEJAMENTO'] == "DDMRP")]
...
def def_exportar_planejado_SAP(df_parametros, df_dado_mestre, base_dir, folder, data_ref):
    ...
    df_parametros = df_parametros.rename(columns={'Material': 'MATERIAL',
                                                  ...
                                                  'UMB': 'UOM'})
    df_parametros = df_parametros[['MATERIAL', 'TIPO', 'DATA', 'QUANTITY', 'UOM']]
...
def def_exportar_planejado_PBI(df_dado_mestre, df_custo, df_parametros,
    ...
    "DAF": "DAF",
    "DIAS_DE_ANTECIPACAO":"DIAS_DE_ANTECIPACAO",
    "TIPO_PLANEJAMENTO":"TIPO_PLANEJAMENTO",
    "DEMANDA_PF":"DEMANDA_PF",
    "QTD_ATRASO_ADJ":"QTD_ATRASO_ADJ",
    "LT PMP":"LT_PMP",
    "DEMANDA_HOJE_REAL":"DEMANDA_HOJE_REAL"}
    ...
    #CALCULAR PLANO_PRIORIDADE
    mask_DDMRP = df_completo["TIPO_PLANEJAMENTO"].eq("DDMRP")

    #Regra DDMRP
    plano_DDMRP = (df_completo["FLUXO_LIQUIDO"]
                   .div(df_completo["TOG"])
                   .where(df_completo["TOG"] != 0, 1))


    #Regra PMP
    plano_PMP = ((df_completo["FLUXO_LIQUIDO"]+df_completo["QTD_ATRASO_ADJ"] + df_completo["DEMANDA_HOJE"] + df_completo["DEMANDA_PF"])
                 .div(df_completo["QTD_ATRASO_ADJ"] + df_completo["DEMANDA_HOJE"] + df_completo["DEMANDA_PF"])
                 .where(df_completo["QTD_ATRASO_ADJ"] + df_completo["DEMANDA_HOJE"] + df_completo["DEMANDA_PF"] != 0, 1))

    # Aplica condicionalmente
    df_completo["PLANO_PRIORIDADE"] = np.where(mask_DDMRP, plano_DDMRP, plano_PMP)

    #DIM_STATUS_DO_BUFFER

    #Condicionamento de Tipo de Planejamento
    mask_DDMRP = df_completo["TIPO_PLANEJAMENTO"].eq("DDMRP")
    mask_PMP = ~mask_DDMRP

    #Caso DDMRP
    tor = df_completo.loc[mask_DDMRP, "TOR"].to_numpy(dtype=float)
    toy = df_completo.loc[mask_DDMRP, "TOY"].to_numpy(dtype=float)
    tog = df_completo.loc[mask_DDMRP, "TOG"].to_numpy(dtype=float)
    prior = df_completo.loc[mask_DDMRP, "PLANO_PRIORIDADE"].to_numpy(dtype=float)

    ratio_tor = np.divide(
        tor,
        tog,
        out=np.full_like(tog, np.inf, dtype=float),
        where=tog != 0)
    ...
    conds_DDMRP = [tog == 0,              # TOG = 0
                   prior == 1,            # PRIOR = 1
                   prior <= 0,            # PRIOR <= 0
                   prior <= ratio_tor,    # PRIOR <= TOR/TOG
                   prior <= ratio_toy,    # PRIOR <= TOY/TOG
                   prior < 1,             # PRIOR < 1
                   prior > 1]             # PRIOR > 1

    choices_DDMRP = ["SEM BUFFER",
                     "NORMAL",
                     "RUPTURA",
                     "RISCO",
                     "BAIXO",
    ...
    df_completo.loc[mask_DDMRP, "DIM_STATUS_DO_BUFFER"] = np.select(conds_DDMRP,
                                                                    choices_DDMRP,
                                                                    default=np.nan)

    #Caso PMP
    fluxo = df_completo.loc[mask_PMP, "PLANO_PRIORIDADE"]
    
    conds_nao = [fluxo.lt(1),   # < 0  -> "BAIXO"
                 fluxo.eq(1),   # = 0  -> "NORMAL"
                 fluxo.gt(1)]   # > 0  -> "EXCESSO"

    choices_nao = ["RUPTURA",
                   "NORMAL",
                   "EXCESSO"]
    
    df_completo.loc[mask_PMP, "DIM_STATUS_DO_BUFFER"] = np.select(conds_nao,
                                                                  choices_nao,
                                                                  default=np.nan)
    ...
    #EXECUCAO_BUFFER onde TIPO_PLANEJAMENTO == "DDMRP"
    df_completo.loc[mask_DDMRP, "EXECUCAO_BUFFER"] = (df_completo.loc[mask_DDMRP, "EST_QTD"]
                                                      .div(df_completo.loc[mask_DDMRP, "TOY"])
                                                      .where(df_completo.loc[mask_DDMRP, "TOY"] != 0, 1))
    
    #EXECUCAO_BUFFER onde TIPO_PLANEJAMENTO == "PMP"
    df_completo.loc[mask_PMP, "EXECUCAO_BUFFER"] = (df_completo.loc[mask_PMP, "EST_QTD"]
                                                      .div(df_completo.loc[mask_PMP, "DEMANDA_HOJE_REAL"])
                                                      .where(df_completo.loc[mask_PMP, "DEMANDA_HOJE_REAL"] != 0, 1))    

    #EXECUCAO_STATUS onde TIPO_PLANEJAMENTO == "DDMRP"
    toy   = df_completo.loc[mask_DDMRP, "TOY"].to_numpy(dtype=float)
    tog   = df_completo.loc[mask_DDMRP, "TOG"].to_numpy(dtype=float)
    tor   = df_completo.loc[mask_DDMRP, "TOR"].to_numpy(dtype=float)
    nivel_de_alerta   = df_completo.loc[mask_DDMRP, "NIVEL_DE_ALERTA"].to_numpy(dtype=float)
    est   = df_completo.loc[mask_DDMRP, "EST_QTD"].to_numpy(dtype=float)
    prior_execucao  = df_completo.loc[mask_DDMRP, "EXECUCAO_BUFFER"].to_numpy(dtype=float)

    ratio_nivel_de_alerta = np.divide(nivel_de_alerta,
                                      toy,
                                      out=np.full_like(toy, np.inf, dtype=float),
                                      where=toy != 0)

    ratio_tor = np.divide(tor,
                          toy,
                          out=np.full_like(toy, np.inf, dtype=float),
                          where=toy != 0)

    conds_DDMRP = [(tog == 0) & (est == 0),                   # TOG = 0 e EST_QTD = 0
                   prior_execucao == 1,                       # EXECUCAO_BUFFER = 1
                   prior_execucao <= 0,                       # EXECUCAO_BUFFER <= 0
                   prior_execucao <= ratio_nivel_de_alerta,   # EXECUCAO_BUFFER <= NIVEL_DE_ALERTA / TOY
                   prior_execucao <= ratio_tor,               # EXECUCAO_BUFFER <= TOR / TOY
                   prior_execucao < 1,                        # EXECUCAO_BUFFER < 1
                   prior_execucao > 1]                        # EXECUCAO_BUFFER > 1

    choices = ["SEM BUFFER", "NORMAL", "RUPTURA", "RISCO", "BAIXO", "NORMAL", "EXCESSO"]

    df_completo.loc[mask_DDMRP, "EXECUCAO_STATUS"] = np.select(conds_DDMRP,
                                                               choices,
                                                               default=np.nan)

    #EXECUCAO_STATUS onde TIPO_PLANEJAMENTO == "PMP"
    diff = df_completo['EST_QTD'] - df_completo['DEMANDA_HOJE_REAL']
    df_completo.loc[mask_PMP & (diff < 0), 'EXECUCAO_STATUS'] = 'RUPTURA'
    df_completo.loc[mask_PMP & (diff == 0), 'EXECUCAO_STATUS'] = 'NORMAL'
    df_completo.loc[mask_PMP & (diff > 0), 'EXECUCAO_STATUS'] = 'EXCESSO'
    ...
    "TOTAL_OPS_ZPS","TOTAL_OPS_ZPT","CATEGORIA","DAF","DIAS_DE_ANTECIPACAO","TIPO_PLANEJAMENTO", "QTD_ATRASO_ADJ",
    "LT_PMP", "DEMANDA_PF", "DEMANDA_HOJE_REAL"]
